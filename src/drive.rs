...
//!
//! Reads are retried freely. Creates use ids pre-generated by Drive, so a retried create can never
//! produce a duplicate or adopt someone else's file: if the first attempt went through, Drive
//! answers 409 and the file is simply fetched. Large uploads query their resumable session and
//! continue from the last byte Drive received. Updates are refused if the file moved, was renamed,
//! or changed since the plan was made.
use crate::auth::Auth;
...
impl Drive {
    ...
    pub fn upload(
        ...
        if size > MULTIPART_LIMIT {
            return self.upload_resumable(&start, &meta, size, local, new_id.as_deref());
        }
